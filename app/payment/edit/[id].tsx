...
export default function EditSubscription() {
  ...
  const [formData, setFormData] = useState<SubscriptionFormData | null>(null);
  ...
  // Cargar datos de la suscripción al montar el componente
  useEffect(() => {
    if (subscription) {
      const loadedFormData: SubscriptionFormData = {
        name: subscription.name,
        startDate: subscription.startDate,
        frequency: subscription.frequency,
        customIntervalDays: subscription.customIntervalDays,
        price: subscription.price.toString(),
        color: subscription.color || predefinedColors[0],
      ...
      const colorIndex = predefinedColors.findIndex(color => color === subscription.color);
      setSelectedColorIndex(colorIndex >= 0 ? colorIndex : 0);
    }
  }, [subscription]);

  if (!subscription || !formData) {
    return (
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.background }]}>
        ...
        <AppText weight="bold" size={20}>{t('edit.title')}</AppText>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <AppText weight="semibold" size={16} style={styles.sectionTitle}>
            {t('basic_info_star')}
          </AppText>
            ...
            <>
              <View style={[styles.inputGroup, {backgroundColor: theme.card}]}>
                ...
                <TextInput
                  ...
                  placeholder=""
                  placeholderTextColor={theme.tabIconDefault}
                  value={formData.notifications.daysBeforePayment?.toString()}
                  onChangeText={(t) => {
                  // opcional: solo dígitos
                  const cleaned = t.replace(/[^\d]/g, '');
                  const n = parseInt(cleaned, 10);
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContainer, { backgroundColor: theme.background }]}>
            ...
            <AppText weight="regular" size={16} style={[styles.modalDescription, { color: theme.tabIconDefault }]}>
              {t('saved.desc', { name: formData.name })}
            </AppText>
